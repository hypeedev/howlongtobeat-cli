...
async fn main() {
    ...
    for (index, game) in res.data.into_iter().enumerate() {
        ...
        let mut formatted_game_name = game.game_name.bold().to_string();
        if (&args.search).join(" ").to_lowercase() == formatted_game_name.to_lowercase() {
            formatted_game_name = formatted_game_name.green().to_string();
        }
        ...
        if game.profile_steam != 0 {
            ...
            formatted_game_name = format!("{} {}", formatted_game_name, link!(url, label));
        }
